...
class CourtHelp(commands.HelpCommand):
    ...
    async def send_command_help(self, command:commands.Command, fake=False, context=None):
        ctx = self.context if context is None else context
        embed = discord.Embed(
        ...
        embed.add_field(
            name="Usage", value=f"`{self.get_command_signature(command, context=context)}`", inline=True
        )
